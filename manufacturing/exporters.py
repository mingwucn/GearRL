...
class ManufacturingExporter:
    """Export certified layouts to auditable manufacturing interchange files."""

    def export_svg(self, train: GearTrain, output_path: str | Path, padding_mm: float = 5.0) -> Path:
        destination = Path(output_path)
        destination.parent.mkdir(parents=True, exist_ok=True)
        min_x, min_y, width, height = self._bounds(train, padding_mm)
        elements = [
    ...
    def export_dxf(self, train: GearTrain, output_path: str | Path) -> Path:
        destination = Path(output_path)
        destination.parent.mkdir(parents=True, exist_ok=True)
