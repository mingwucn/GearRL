...
A separate preregistered selection study trains a robustness selector on four randomized-QMC replicates, compares it with a compactness-only nominal selector, and evaluates both selected sets on eight disjoint replicates. The primary claim requires the held-out 95% interval for probability improvement to exceed 0.005.
...
Robustness-trained selection raises held-out modeled acceptance from 0.02814 to 0.03141, an improvement of 0.00327 with a 95% interval of [0.00296, 0.00358]. Because the interval does not exceed the preregistered 0.005 threshold, the primary tolerance-aware selection claim is not supported.
...
**Registered table: `tolerance-aware-selection`**

| Nominal probability | Robust-selected probability | Improvement | Held-out 95% CI | Required improvement | Supported |
| ---: | ---: | ---: | ---: | ---: | --- |
| 0.02814 | 0.03141 | 0.00327 | [0.00296, 0.00358] | 0.00500 | No |

![solver-anytime-largest-domain](../generated-v3/figures/solver-anytime-largest-domain.svg)

![assembly-robustness-response](../generated-v3/figures/assembly-robustness-response.svg)
...
The assembly study shows that nominal validity does not imply tolerance robustness. Under the declared rigid-center model, backlash can admit only positive radial separation within the standard small-displacement allowance; it cannot repair contractions or arbitrary two-mesh error combinations. The held-out selection study finds a real but sub-threshold improvement, indicating that ranking existing nominal layouts is insufficient. Assembly offsets or tolerance margins must become explicit synthesis decisions before a tolerance-aware design claim is supportable.
