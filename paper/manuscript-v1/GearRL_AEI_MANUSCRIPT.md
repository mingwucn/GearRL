...
Prior work already establishes knowledge-graph-driven architecture generation [8,10], graph-based transmission configuration synthesis [6], constrained simulation optimization [1], automated gearbox sizing [2], and learning-guided combinatorial design [7]. Therefore, graph use, optimization, and simulation coupling are not claimed as novel individually. Within the audited closest-method set, the unresolved combination is a solver-hidden requirements benchmark, executable gear-domain knowledge, independent positive certificates and complete negative proofs, strength-coupled admission, and hash-bound reporting in one workflow.
