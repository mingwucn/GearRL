...
Prior work already establishes knowledge-graph-driven architecture generation \cite{huang-2025,sun-2026}, graph-based transmission configuration synthesis \cite{geng-2023}, constrained simulation optimization \cite{cicconi-2020}, automated gearbox sizing \cite{fuerst-2021}, and learning-guided combinatorial design \cite{lin-2023}. Therefore, graph use, optimization, and simulation coupling are not claimed as novel individually. Within the audited closest-method set, the unresolved combination is a solver-hidden requirements benchmark, centralized executable gear-domain semantics, independent positive certificates and complete negative proofs, adversarial representation ablation, and hash-bound reporting in one workflow.
...
\small
\begin{tabular}{lllll}
\textbf{Representation} & \textbf{Competency accuracy} & \textbf{Mutation detection} & \textbf{Exact localization} & \textbf{Semantic rule sites} \\
\hline
...
\small
\begin{tabular}{lllll}
\textbf{Runs} & \textbf{Constraint-valid} & \textbf{At acceptance threshold} & \textbf{Best objective} & \textbf{Conversion status} \\
\hline
...
\begin{table}[htbp]
\centering
\caption{Tolerance Aware Selection}
\small
\begin{tabular}{llllll}
\textbf{Nominal probability} & \textbf{Robust-selected probability} & \textbf{Improvement} & \textbf{Held-out 95\% CI} & \textbf{Required improvement} & \textbf{Supported} \\
\hline
...
\caption{Modeled-valid probability across preregistered center-distance backlash increments and shaft-location tolerances at zero housing erosion. Probabilities are digital-model outcomes, not measured manufacturing yield.}
\end{figure}
\section{Discussion}
