...
def aggregate_bundle(bundle: str | Path) -> dict[str, Any]:
    root = Path(bundle)
...
def write_aggregate(bundle: str | Path) -> Path:
    root = Path(bundle)
    destination = root / "summary.json"
    destination.write_text(json.dumps(aggregate_bundle(root), indent=2, sort_keys=True) + "\n")
    return destination
