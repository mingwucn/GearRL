...
class TestAEISubmissionPackage:
    ...
    def test_package_detects_tampering(self, tmp_path):
        root = tmp_path / "package"
        store = AEISubmissionPackageStore()
        store.build(Path("paper/manuscript_source.json"), Path("paper/aei_submission_source.json"), Path("literature/aei_closest_methods.json"), Path("paper/generated-v3"), root)
