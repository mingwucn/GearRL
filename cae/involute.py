...
def involute_point(base_radius_mm: float, roll_angle_rad: float) -> tuple[float, float]:
...
def involute_flank(module_mm: float, teeth: int, pressure_angle_deg: float = 20.0, samples: int = 24) -> np.ndarray:
    ...
    return np.asarray([involute_point(base_radius, roll) for roll in rolls])


def pitch_radius_mm(module_mm: float, teeth: int) -> float:
